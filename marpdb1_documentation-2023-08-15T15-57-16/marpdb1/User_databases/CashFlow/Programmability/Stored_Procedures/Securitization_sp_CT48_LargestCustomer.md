...
[Project](../../../../../index.md) > [marpdb1](../../../../index.md) > [User databases](../../../index.md) > [CashFlow](../../index.md) > Programmability > [Stored Procedures](Stored_Procedures.md) > Securitization.sp_CT48_LargestCustomer

# ![Stored Procedures](../../../../../Images/StoredProcedure32.png) [Securitization].[sp_CT48_LargestCustomer]
...
-- =============================================
CREATE PROCEDURE [Securitization].[sp_CT48_LargestCustomer]
	@PoolID [int],
	@UserID [nvarchar](255)
AS
...
SELECT @ConcentrationTest_ID = 48
...
WITH Ranks AS (
select [CustomerID] as RankField  from(
select RTrim([CustomerID]) as [CustomerID], 
...
INNER JOIN [Securitization].[Contracts] c on pc.Contract_ContractID = c.ContractID
INNER JOIN Ranks r ON r.RankField = RTRIM(c.CustomerID)
WHERE pc.Pool_ID = @PoolID
AND pc.Active = 1
	AND (
	      (
