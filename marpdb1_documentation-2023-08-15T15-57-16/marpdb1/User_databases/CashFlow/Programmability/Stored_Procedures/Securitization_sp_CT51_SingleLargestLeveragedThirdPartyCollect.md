...
[Project](../../../../../index.md) > [marpdb1](../../../../index.md) > [User databases](../../../index.md) > [CashFlow](../../index.md) > Programmability > [Stored Procedures](Stored_Procedures.md) > Securitization.sp_CT51_SingleLargestLeveragedThirdPartyCollect

# ![Stored Procedures](../../../../../Images/StoredProcedure32.png) [Securitization].[sp_CT51_SingleLargestLeveragedThirdPartyCollect]
...
-- =============================================
CREATE PROCEDURE [Securitization].[sp_CT51_SingleLargestLeveragedThirdPartyCollect]
	@PoolID [int],
	@UserID [nvarchar](255)
AS
...
SELECT @ConcentrationTest_ID = 51
...
WITH Ranks AS (
select [IndirectBillCollectVendorID] as RankField  from(
select RTrim([IndirectBillCollectVendorID]) as [IndirectBillCollectVendorID], 
...
AND pc.Active = 1 
AND [LeveragedIndirectBillCollect] = 'Y'
group by RTrim([IndirectBillCollectVendorID]) 
...
INNER JOIN [Securitization].[Contracts] c on pc.Contract_ContractID = c.ContractID
INNER JOIN Ranks r ON r.RankField = RTRIM(c.IndirectBillCollectVendorID)
WHERE pc.Pool_ID = @PoolID
AND pc.Active = 1
	AND (
	      (
[LeveragedIndirectBillCollect] = 'Y'	      )
) OPTION (RECOMPILE);
WITH Ranks AS (
select [IndirectBillCollectVendorID] as RankField from(
select RTrim([IndirectBillCollectVendorID]) as [IndirectBillCollectVendorID], 
