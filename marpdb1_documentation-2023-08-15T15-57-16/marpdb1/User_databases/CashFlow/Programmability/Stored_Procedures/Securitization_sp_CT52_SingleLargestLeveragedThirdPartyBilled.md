...
[Project](../../../../../index.md) > [marpdb1](../../../../index.md) > [User databases](../../../index.md) > [CashFlow](../../index.md) > Programmability > [Stored Procedures](Stored_Procedures.md) > Securitization.sp_CT52_SingleLargestLeveragedThirdPartyBilled

# ![Stored Procedures](../../../../../Images/StoredProcedure32.png) [Securitization].[sp_CT52_SingleLargestLeveragedThirdPartyBilled]
...
-- =============================================
CREATE PROCEDURE [Securitization].[sp_CT52_SingleLargestLeveragedThirdPartyBilled]
	@PoolID [int],
	@UserID [nvarchar](255)
AS
...
SELECT @ConcentrationTest_ID = 52
...
WITH Ranks AS (
select [AggregationParentID] as RankField  from(
select RTrim([AggregationParentID]) as [AggregationParentID], 
...
AND pc.Active = 1 
AND [IndirectBilling] = 'B' OR [Concentrations] = 'L'
group by RTrim([AggregationParentID]) 
...
INNER JOIN [Securitization].[Contracts] c on pc.Contract_ContractID = c.ContractID
INNER JOIN Ranks r ON r.RankField = RTRIM(c.AggregationParentID)
WHERE pc.Pool_ID = @PoolID
AND pc.Active = 1
	AND (
	      (
[IndirectBilling] = 'B' OR [Concentrations] = 'L'	      )
) OPTION (RECOMPILE);
WITH Ranks AS (
select [AggregationParentID] as RankField from(
select RTrim([AggregationParentID]) as [AggregationParentID], 
