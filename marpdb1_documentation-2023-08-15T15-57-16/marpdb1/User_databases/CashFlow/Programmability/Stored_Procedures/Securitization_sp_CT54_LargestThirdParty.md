...
[Project](../../../../../index.md) > [marpdb1](../../../../index.md) > [User databases](../../../index.md) > [CashFlow](../../index.md) > Programmability > [Stored Procedures](Stored_Procedures.md) > Securitization.sp_CT54_LargestThirdParty

# ![Stored Procedures](../../../../../Images/StoredProcedure32.png) [Securitization].[sp_CT54_LargestThirdParty]
...
-- =============================================
CREATE PROCEDURE [Securitization].[sp_CT54_LargestThirdParty]
	@PoolID [int],
	@UserID [nvarchar](255)
AS
...
SELECT @ConcentrationTest_ID = 54
...
WITH Ranks AS (
select [DealerID] as RankField  from(
select RTrim([DealerID]) as [DealerID], 
ROW_NUMBER() OVER(ORDER BY SUM(COALESCE(pc.NPV,0)) DESC) as RankPosition 
FROM [Securitization].[PoolContract] pc 
INNER JOIN [Securitization].[Contracts] c on pc.Contract_ContractID = c.ContractID 
WHERE pc.Pool_ID = @PoolID 
AND pc.Active = 1 
AND RTrim([DealerID] ) IN (SELECT [ItemText] FROM [Securitization].[ListItems] WHERE [List_ID] = (SELECT TOP 1 ID FROM [Securitization].[Lists] WHERE ShortName = 'ThirdParty'))
group by RTrim([DealerID]) 
...
INNER JOIN [Securitization].[Contracts] c on pc.Contract_ContractID = c.ContractID
INNER JOIN Ranks r ON r.RankField = RTRIM(c.DealerID)
WHERE pc.Pool_ID = @PoolID
AND pc.Active = 1
	AND (
	      (
RTrim([DealerID] ) IN (SELECT [ItemText] FROM [Securitization].[ListItems] WHERE [List_ID] = (SELECT TOP 1 ID FROM [Securitization].[Lists] WHERE ShortName = 'ThirdParty'))	      )
) OPTION (RECOMPILE);
WITH Ranks AS (
select [DealerID] as RankField from(
select RTrim([DealerID]) as [DealerID], 
