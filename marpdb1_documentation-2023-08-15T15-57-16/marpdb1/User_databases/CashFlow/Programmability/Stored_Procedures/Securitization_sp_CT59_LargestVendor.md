...
[Project](../../../../../index.md) > [marpdb1](../../../../index.md) > [User databases](../../../index.md) > [CashFlow](../../index.md) > Programmability > [Stored Procedures](Stored_Procedures.md) > Securitization.sp_CT59_LargestVendor

# ![Stored Procedures](../../../../../Images/StoredProcedure32.png) [Securitization].[sp_CT59_LargestVendor]
...
-- =============================================
CREATE PROCEDURE [Securitization].[sp_CT59_LargestVendor]
	@PoolID [int],
	@UserID [nvarchar](255)
AS
...
SELECT @ConcentrationTest_ID = 59
...
WITH Ranks AS (
select [AggregationParentID_WithExceptions] as RankField  from(
select RTrim([AggregationParentID_WithExceptions]) as [AggregationParentID_WithExceptions], 
...
AND pc.Active = 1 
AND ([AggregationParentID_WithExceptions] IS NOT NULL AND [AggregationParentID_WithExceptions] <> '')
group by RTrim([AggregationParentID_WithExceptions]) 
...
INNER JOIN [Securitization].[Contracts] c on pc.Contract_ContractID = c.ContractID
INNER JOIN Ranks r ON r.RankField = RTRIM(c.AggregationParentID_WithExceptions)
WHERE pc.Pool_ID = @PoolID
AND pc.Active = 1
	AND (
	      (
([AggregationParentID_WithExceptions] IS NOT NULL AND [AggregationParentID_WithExceptions] <> '')	      )
) OPTION (RECOMPILE);
WITH Ranks AS (
select [AggregationParentID_WithExceptions] as RankField from(
select RTrim([AggregationParentID_WithExceptions]) as [AggregationParentID_WithExceptions], 
