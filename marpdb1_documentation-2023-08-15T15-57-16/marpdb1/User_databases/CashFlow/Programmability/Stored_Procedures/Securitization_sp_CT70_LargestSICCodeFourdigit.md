...
[Project](../../../../../index.md) > [marpdb1](../../../../index.md) > [User databases](../../../index.md) > [CashFlow](../../index.md) > Programmability > [Stored Procedures](Stored_Procedures.md) > Securitization.sp_CT70_LargestSICCodeFourdigit

# ![Stored Procedures](../../../../../Images/StoredProcedure32.png) [Securitization].[sp_CT70_LargestSICCodeFourdigit]
...
-- =============================================
CREATE PROCEDURE [Securitization].[sp_CT70_LargestSICCodeFourdigit]
	@PoolID [int],
	@UserID [nvarchar](255)
AS
...
SELECT @ConcentrationTest_ID = 70
...
WITH Ranks AS (
select Left([SicCode],4) as RankField  from(
select RTrim(Left([SicCode],4)) as [SicCode], 
...
AND pc.Active = 1 
AND ([SicCode] IS NOT NULL AND [SicCode] <> '')
group by RTrim(Left([SicCode],4)) 
...
INNER JOIN [Securitization].[Contracts] c on pc.Contract_ContractID = c.ContractID
INNER JOIN Ranks r ON r.RankField = RTRIM(c.SicCode)
WHERE pc.Pool_ID = @PoolID
AND pc.Active = 1
	AND (
	      (
([SicCode] IS NOT NULL AND [SicCode] <> '')	      )
) OPTION (RECOMPILE);
WITH Ranks AS (
select Left([SicCode],4) as RankField from(
select RTrim(Left([SicCode],4)) as [SicCode], 
