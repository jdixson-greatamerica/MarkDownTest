...
[Project](../../../../../index.md) > [marpdb1](../../../../index.md) > [User databases](../../../index.md) > [CashFlow](../../index.md) > Programmability > [Stored Procedures](Stored_Procedures.md) > Securitization.sp_Facility_CT18_TotalnonmonthlyQSkipsASAVDeferredGAFI

# ![Stored Procedures](../../../../../Images/StoredProcedure32.png) [Securitization].[sp_Facility_CT18_TotalnonmonthlyQSkipsASAVDeferredGAFI]
...
-- =============================================
CREATE PROCEDURE [Securitization].[sp_Facility_CT18_TotalnonmonthlyQSkipsASAVDeferredGAFI]
	@FacilityID [int],
	@UserID [nvarchar](255)
AS
...
SELECT @ConcentrationTest_ID = 18
...
WHERE c.Lessor in (SELECT fl.LessorID from Securitization.Facilities f INNER JOIN Securitization.FacilityLessor fl on f.id = fl.Facility_ID where f.id = @FacilityID)
	AND (	      (
(RTrim([BillingFrequency] ) IN ('Quarterly','Skip Pymt','Annual','Semi-Annual') OR [Concentrations] LIKE '%V%') OR ([DaysDeferred] > 0) 
AND NOT (DATEDIFF(day, [BookingDate], GETDATE()) > DaysDeferred OR DATEDIFF(day, [CommenceDate], GETDATE()) > DaysDeferred AND [PymtsMade] > 0))	      
) 
